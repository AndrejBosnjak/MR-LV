...
def evaluateInd(individual):
    ...
    for i in range(0,NO_DIMS):
        fit_val += individual[i]**2 - A*math.cos(2*math.pi*individual[i])
...
class Ui_MainWindow(QtWidgets.QMainWindow):
    ...
    def btnStartGA_Click(self, nGen=NGEN, popSize = POP_SIZE, mutationRate = GA_MUTPB, numOfElites = GA_NELT, maxAbs = GA_MAX_ABS):
    ...
    def btnStartPSO_Click(self, nGen=NGEN, popSize = POP_SIZE, inertia = PSO_INERTIA, personal = PSO_PERSONAL, social = PSO_SOCIAL):
        #Set global variables
        global stop_evolution
        global q_min_series
        global q_max_series
    ...
    def autorun(self):
        ...
        for M_index in range(3):
            self.runGA(POP=0, E=0, M=M_index, generationCount=nGen, maxA=0)
        for E_index in range(3):
            self.runGA(POP=0, E=E_index, M=0, generationCount=nGen, maxA=0)
        for maxAbs_index in range(3):
            self.runGA(POP=0, E=0, M=0, generationCount=nGen, maxA=maxAbs_index)
        for I_index in range(3):
            self.runPSO(POP=0, I=I_index, P=0, S=0, generationCount=nGen)
        for P_index in range(3):
            self.runPSO(POP=0, I=0, P=P_index, S=0, generationCount=nGen)
        for S_index in range(3):
            self.runPSO(POP=0, I=0, P=0, S=S_index, generationCount=nGen)

    def runGA(self, POP, E, M, generationCount, maxA):
        populationSize = [100]
        numOfElites = [4, 8, 16]
        mutationRate = [0.05, 0.10, 0.20]
        maxAbs = [0.1, 0.4, 0.8]
        ...
        for i in range(5):
            self.btnStartGA_Click(
                                popSize=populationSize[POP],
                                numOfElites=numOfElites[E],
                                mutationRate=mutationRate[M],
                                nGen=generationCount,
                                maxAbs=maxAbs[maxA])
            values = []
            for i in range(q_min_series.count()):
            ...
            best_ind = tools.selBest(self.pop, 1)[0]
            fitnessValues.append(best_ind.fitness.values)
            bestIndividual=np.asarray(best_ind)
            bestIndividual=np.round(bestIndividual, 2)
            statsData.append(bestIndividual)
        graphData.sort(key=minValue)
        indicies=np.argsort([i[0] for i in fitnessValues])
        statsData = [statsData[i] for i in indicies]

        medianFileName = gaFolderName + "graphData/" + str(NO_DIMS) + "_Dimensions&" + str(mutationRate[M]) + "_mutationProb&" + str(numOfElites[E]) + "_numOfElites&" + str(maxAbs[maxA]) + "_maxMutatedValue_median.csv"
        bestFileName = gaFolderName + "graphData/" + str(NO_DIMS) + "_Dimensions&" + str(mutationRate[M]) + "_mutationProb&" + str(numOfElites[E]) + "_numOfElites&" + str(maxAbs[maxA]) + "_maxMutatedValue_best.csv"
        ...
        statsFname = gaFolderName + "data/" + str(NO_DIMS) + "_Dimensions&" + str(mutationRate[M]) + "_mutationProb&" + str(numOfElites[E]) + "_numOfElites&" + str(maxAbs[maxA]) + "_maxMutatedValue.txt"
        stats = str(NO_DIMS) + "_Dimensions&" + str(mutationRate[M]) + "_mutationProb&" + str(numOfElites[E]) + "_numOfElites&" + str(maxAbs[maxA]) + "_maxMutatedValue"
        stats += "\nValues At the end are: \n"
        for i in range(5):
            stats += str(statsData[i]) + ", fitness value= " + str(graphData[i][-1]) + "\n"
        with open(statsFname, 'w') as dat:
            dat.write(stats)



    def runPSO(self, POP, I, P, S, generationCount):
        populationSize = [100]
        inertia = [0.0, 0.37, 0.74]
        personal = [0.5, 1, 1.5]
        social = [0.5, 1, 1.5]
        graphData = []
        statsData = []
        ...
        for i in range(5):
            ...
            best_ind = tools.selBest(self.pop, 1)[0]
            fitnessValues.append(best_ind.fitness.values)
            bestIndividual=np.asarray(best_ind.best)
            bestIndividual=np.round(bestIndividual, 2)
            statsData.append(bestIndividual)
        graphData.sort(key=minValue)
        indicies=np.argsort([i[0] for i in fitnessValues])
        ...
        medianFileName = psoFolderName + "graphData/" + str(NO_DIMS) + "_Dimensions&" + str(inertia[I]) + "_InertiaVal&" + str(personal[P]) + "_individualFactor&" + str(social[S]) + "_socialFactor_median.csv"
        bestFileName =  psoFolderName + "graphData/" + str(NO_DIMS) + "_Dimensions&" + str(inertia[I]) + "_InertiaVal&" + str(personal[P]) + "_individualFactor&" + str(social[S]) + "_socialFactor_median.csv"
        ...
        statsFname = psoFolderName + "data/" + str(NO_DIMS) + "_Dimensions&" + str(inertia[I]) + "_InertiaVal&" + str(personal[P]) + "_individualFactor&" + str(social[S]) + "_socialFactor_median.txt"
        stats = str(NO_DIMS) + "_Dimensions&" + str(inertia[I]) + "_InertiaVal&" + str(personal[P]) + "_individualFactor&" + str(social[S]) + "_socialFactor"
        stats += "\nValues At the end are: \n"
        for i in range(5):
            stats += str(statsData[i]) + ", fitness value= " + str(graphData[i][-1]) + "\n"
        with open(statsFname, 'w') as dat:
