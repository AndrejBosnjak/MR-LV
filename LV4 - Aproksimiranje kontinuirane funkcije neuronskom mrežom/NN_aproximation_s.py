...
N_PER_LAYER = (10,10) #Neurons per layer (10 neurona u prvom, 10 u drugom sloju)
...
class Ui_MainWindow(QtWidgets.QMainWindow):
    ...
    def btnStart_Click(self, nSample,nNeurons,nLayers,activationFunc,solverFunc):   
        ...
        #Create neural network (multi layered perecptron)
        ...
        mlp =  MLPRegressor(hidden_layer_sizes=N_PER_LAYER, activation=a_functions[ACTIVATION_F], alpha=ALPHA, solver=solvers[SOLVER], max_iter=MAX_ITER)
        ...
        if(GUI):
            ...
            x = list(range(0,75))
            plt.figure()
            plt.plot(x, test_data[:,1])
            plt.plot(x, output)
            plt.title("{}nSamples_{}nNeurons_{}nLayers_{}_{}".format(nSample, nNeurons, nLayers, a_functions[ACTIVATION_F], solvers[SOLVER]))
            plt.savefig("graphs/{}nSamples_{}nNeurons_{}nLayers_{}_{}".format(nSample, nNeurons, nLayers, a_functions[ACTIVATION_F], solvers[SOLVER]))
    ...
    def autorun(self):
        ...
        if not os.path.exists("MSEValues/"):
            ...
            open("MSEValues/MSEValues.csv", "x")

        for nSample in nSample_:
            for nNeurons in nNeurons_:
                for nLayers in nLayers_:
                    for activationFunc in activationFunc_:
                        for solverFunc in solverFunc_:
                            mse = self.btnStart_Click(nSample=nSample, nNeurons=nNeurons, nLayers=nLayers, activationFunc=activationFunc, solverFunc=solverFunc)
                            with open("MSEValues/MSEValues.csv", "a") as file:
                                file.write("{}nSamples_{}nNeurons_{}nLayers_{}_{}".format(nSample, nNeurons, nLayers, a_functions[activationFunc], solvers[solverFunc]) + "," + str(mse) + "\n")
