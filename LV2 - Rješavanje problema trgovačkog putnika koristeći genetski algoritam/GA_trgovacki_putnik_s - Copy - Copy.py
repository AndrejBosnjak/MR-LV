...
class Ui_MainWindow(QtWidgets.QMainWindow):
    ...
    def run(self, POP, E, M, generationCount):
        ...
        graphDataFname = "graphData/BorderPatrol=False"  + "_popSize=" + str(populationSize[POP]) + "_numElites=" + str(numOfElites[E]) + "_mutationRate=" + str(mutationRate[M]) + ".csv"
        ...
        statsfName = "stats/" + "False_borderPatrol_" + str(populationSize[POP]) + "_popSize_" + str(numOfElites[E]) + "_elites_" + str(mutationRate[M]) + "_mutationRate_.txt"
        stats = "Border patrol is False" + ", " + str(populationSize[POP]) + " Population Size, " + str(numOfElites[E]) + " Number of Elites & " + str(mutationRate[M]) + " Mutation Rate" + "\nAvgBestPath=" + str(round(avg,1)) + "\nBestIndividual=" + str(bestIndividual) + "\nBest paths=%i,%i,%i,%i,%i" % (shortestPaths[0], shortestPaths[1], shortestPaths[2], shortestPaths[3], shortestPaths[4])  
        with open(statsfName, 'w') as dat:
            dat.write(stats)
        print("Stats for False border Patrol, population size of %i, %i number of elites and %f percent mutation rate run written in %s file" % (populationSize[POP], numOfElites[E], mutationRate[M], statsfName))
...
if __name__ == "__main__":
    ...
    ui = Ui_MainWindow()
    img = generateWorldImage([0, 10, 11, 4, 2, 8, 6, 12, 7, 3, 16, 17, 13, 19, 14, 20, 9, 1, 5, 18, 15])
    img.save("C:\Fakultet\FERIT\8. semestar\Meko računarstvo\LV\LV2\put.png")

    img1=Image.open("Croatia620.png")
    img2=Image.open("put.png")
    img1.paste(img2, (0,0), mask = img2)
    img1.show()
    img1.save("C:\Fakultet\FERIT\8. semestar\Meko računarstvo\LV\LV2\put2.png")
    if GUI:     
        ui.setupUi()
        ui.show()
        sys.exit(app.exec_())
